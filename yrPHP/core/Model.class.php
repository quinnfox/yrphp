...
class Model {
	...
	public function __construct($tablePrefix = '', $tableName = '') {
		...
		if (empty($db['slaveServer'])) {
			...
			if (!is_array($db['slaveServer'])) {
				$db['slaveServer'] = array($db['slaveServer']);
			}
			...
			foreach ($where as $k => $v) {
				...
				if (is_array($v)) {
					...
					if (strripos($symbol, 'is') !== false) {
						$value = $value;
					} elseif (strripos($symbol, 'in') !== false) {
//in || not in
						$value = '(' . $value . ')';
					} elseif (strripos($symbol, 'between') !== false) {
//between|not between
	...
	public final function check($array, $tableName = "") {
		...
		foreach ($array as $key => $value) {
			...
			if (in_array(strtolower($key), array_map('strtolower', $tableField))) {
//判断字段是否存在 不存在则舍弃
